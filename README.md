...
1. Check whether the player's action references a known item/character by name
2. If so, query the Chroma vector store for that entity's lore
3. Feed the retrieved lore (if any) into the LLM prompt
4. Generate narration and update game state
...
Settings → System → About → Advanced system settings → Environment Variables → New (User variable) → `GOOGLE_API_KEY`
...
1. Choose a character from the selection menu
2. Type actions in plain English ("I attack the goblin", "I search the room", "I grab the sword")
3. The game classifies your action, routes it to the right agent, and narrates the outcome
4. Game ends when HP reaches 0 (death) or inventory reaches 3 items (win)
